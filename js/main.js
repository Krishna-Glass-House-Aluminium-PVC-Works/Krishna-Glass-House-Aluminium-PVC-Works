...
- Always answer in the language the user writes in.`;
  ...
  const reportIcon = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/></svg>`;
  ...
  function appendMsg(role, html) {
    ...
    if (role === 'bot') {
      wrap.innerHTML = `
        ${botIcon()}
        <div class="kgh-msg-bubble">${html}</div>
        <button class="kgh-msg-report" aria-label="Report this message" title="Report message">
          ${reportIcon}
        </button>
      `;
      wrap.querySelector('.kgh-msg-report').addEventListener('click', () => {
  ...
  fab.addEventListener('click', (e) => {
    e.stopPropagation(); // Stop bubbling to prevent document outside-click from firing
  ...
  /* ── Report Modal Support ── */
  const reportOverlay = document.createElement('div');
  reportOverlay.className = 'kgh-report-overlay';
  reportOverlay.innerHTML = `
    <div class="kgh-report-modal">
      <h3>Report Content</h3>
      <p>We take content safety seriously. Please provide your email and the reason for reporting this AI response:</p>
      <div class="kgh-report-email-wrap">
        <label for="kgh-report-email">Your Email Address</label>
        <input type="email" id="kgh-report-email" placeholder="email@example.com" required>
      </div>
      <div class="kgh-report-options">
        <label><input type="radio" name="report-reason" value="Inaccurate" checked> Inaccurate or misleading</label>
        <label><input type="radio" name="report-reason" value="Offensive"> Offensive or inappropriate</label>
        <label><input type="radio" name="report-reason" value="Spam"> Spam or advertisement</label>
        <label><input type="radio" name="report-reason" value="Other"> Other / Privacy concern</label>
      </div>
      <div class="kgh-report-actions">
        <button class="kgh-report-cancel" id="kgh-report-cancel">Cancel</button>
        <button class="kgh-report-submit" id="kgh-report-submit">Submit Report</button>
      </div>
    </div>
  `;
  document.body.appendChild(reportOverlay);

  const reportEmailInput = document.getElementById('kgh-report-email');

  function showReportModal(content) {
    reportOverlay.classList.add('visible');
    reportOverlay.dataset.reportedContent = content;
  ...
  document.getElementById('kgh-report-cancel').addEventListener('click', closeReportModal);
  document.getElementById('kgh-report-submit').addEventListener('click', () => {
    const email = reportEmailInput.value.trim();
    const reason = reportOverlay.querySelector('input[name="report-reason"]:checked').value;

    if (!email || !email.includes('@')) {
      reportEmailInput.style.borderColor = '#ef4444';
      reportEmailInput.focus();
      return;
    }
    reportEmailInput.style.borderColor = '';

    // In a real app, you would send this to your backend/API
    console.log('User reported AI message:', reportOverlay.dataset.reportedContent);
    console.log('User Email:', email);
    console.log('Reason:', reason);

    const submitBtn = document.getElementById('kgh-report-submit');
